 ...
 * 开发平台-表单类型管理
*/
@Api(value = "开发平台-表单类型管理", description = "开发平台-表单类型管理")
@Path(RestConstants.RestPathPrefix.DPFORMTYPE)
public class DpFormTypeRestServer extends BaseRestServerInterfaceImpl<DpFormType> {
	@Override
	public IDpFormTypeProvider getDubboBaseInterface() {
		return ControlConsumer.getDpFormTypeProvider();
	}

	/**
	 * 表单设计器分类删除验证，若分类下有数据则不允许删除
	 * @param ids
	 * @return
	...
	public JsonViewObject deleteByIds(@ApiParam(value = "多个记录id，用逗号分隔", required = true, defaultValue = "", example = "1,2") @QueryParam("ids") String ids){
		...
		try {
			...
			log.error("DpFormTypeRestServer deleteByIds is error,{Id:" + ids + "}", e);
		}
		return jsonView;
	}
}
