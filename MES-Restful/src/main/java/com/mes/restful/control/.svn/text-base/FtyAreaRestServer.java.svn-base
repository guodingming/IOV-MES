...
@Api(value = "工厂管理-车间管理", description = "工厂管理-车间管理")
@Path(RestConstants.RestPathPrefix.FTYAREA)
public class FtyAreaRestServer extends BaseRestServerInterfaceImpl<FtyArea> {
	@Override
	public IFtyAreaProvider getDubboBaseInterface() {
		return ControlConsumer.getFtyAreaProvider();
	}
	...
	public JsonViewObject deleteByIds(@ApiParam(value = "多个记录id，用逗号分隔", required = true, defaultValue = "", example = "1,2") @QueryParam("ids") String ids){
		...
		try {
			...
			log.error("FtyAreaRestServer deleteByIds is error,{Id:" + ids + "}", e);
		}
		return jsonView;
	}
}
