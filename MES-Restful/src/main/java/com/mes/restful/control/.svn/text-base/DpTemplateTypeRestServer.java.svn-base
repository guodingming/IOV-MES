 ...
 * 开发平台-模板分类
*/
@Api(value = "开发平台-模板分类", description = "开发平台-模板分类")
@Path(RestConstants.RestPathPrefix.DPTEMPLATETYPE)
public class DpTemplateTypeRestServer extends BaseRestServerInterfaceImpl<DpTemplateType> {
	@Override
	public IDpTemplateTypeProvider getDubboBaseInterface() {
		return ControlConsumer.getDpTemplateTypeProvider();
	}

	/**
	 * 模板管理分类删除验证，若分类下有数据则不允许删除
	 * @param ids
	 * @return
	...
	public JsonViewObject deleteByIds(@ApiParam(value = "多个记录id，用逗号分隔", required = true, defaultValue = "", example = "1,2") @QueryParam("ids") String ids){
		...
		try {
			...
			log.error("DpTemplateTypeRestServer deleteByIds is error,{Id:" + ids + "}", e);
		}
		return jsonView;
	}
}
