 ...
 * 开发平台-素材分类
*/
@Api(value = "开发平台-素材分类", description = "开发平台-素材分类")
@Path(RestConstants.RestPathPrefix.DPMATERIALTYPE)
public class DpMaterialTypeRestServer extends BaseRestServerInterfaceImpl<DpMaterialType> {
	@Override
	public IDpMaterialTypeProvider getDubboBaseInterface() {
		return ControlConsumer.getDpMaterialTypeProvider();
	}

	/**
	 * 素材管理分类删除验证，若分类下有数据则不允许删除
	 * @param ids
	 * @return
	...
	public JsonViewObject deleteByIds(@ApiParam(value = "多个记录id，用逗号分隔", required = true, defaultValue = "", example = "1,2") @QueryParam("ids") String ids){
		...
		try {
			...
			log.error("DpMaterialTypeRestServer deleteByIds is error,{Id:" + ids + "}", e);
		}
		return jsonView;
	}
}
